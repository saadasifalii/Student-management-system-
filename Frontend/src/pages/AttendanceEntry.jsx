...
function AttendanceEntry() {
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                ...
                let offeringsData = extractList(offeringsRes.data);
                const coursesData = extractList(coursesRes.data);

                if (!isAdmin) {
                    const teachersRes = await api.get('/teachers?limit=1000');
                    const teachersData = extractList(teachersRes.data);
                    const myTeacherRecord = teachersData.find((t) => t.user_id === user.id);
                    offeringsData = offeringsData.filter((o) => o.teacher_id === myTeacherRecord?.id);
                }
    ...
    const loadClass = async () => {
        ...
        try {
            ...
            const attendance = extractList(attendanceRes.data);
    ...
    const updateRow = (studentId, field, value) => {
        setRows((currentRows) =>
            currentRows.map((r) => (r.studentId === studentId ? { ...r, [field]: value } : r))
        );
    };

        ...
        const fetchDashboardData = async () => {
            try {
                if (user.role === 'admin') {
                    ...
                    setStats({
                        totalStudents: extractList(studentsRes.data).length,
                        totalTeachers: extractList(teachersRes.data).length,
                        totalCourses: extractList(coursesRes.data).length,
                        pendingDocuments: extractList(documentsRes.data).filter((d) => d.status === 'pending').length,
                    });
                    ...
                    const offeringsList = extractList(offeringsRes.data.courseOfferings || offeringsRes.data);
                    const teachersList = extractList(teachersRes.data);
                    const coursesList = extractList(coursesRes.data);

                    const myTeacherRecord = teachersList.find((t) => t.user_id === user.id);
                    const myOfferings = offeringsList.filter((o) => o.teacher_id === myTeacherRecord?.id);
                    const withCourseNames = myOfferings.map((o) => {
                        const course = coursesList.find((c) => c.id === o.course_id);
                    ...
                    const enrollmentsList = extractList(enrollmentsRes.data);
                    const offeringsList = extractList(offeringsRes.data.courseOfferings || offeringsRes.data);
                    const coursesList = extractList(coursesRes.data);

                    const myEnrollments = enrollmentsList.filter((e) => e.student_id === myStudent.id && e.status === 'enrolled');
                    const myCourses = myEnrollments.map((e) => {
                        const offering = offeringsList.find((o) => o.id === e.course_offering_id);
                        const course = offering ? coursesList.find((c) => c.id === offering.course_id) : null;
